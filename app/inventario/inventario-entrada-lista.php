    ...
    <script>
        ...
        function cargarEntradas() {
            ...
            fetch(url)
                ...
                .catch(error => {
                    document.getElementById('loading').style.display = 'none';
                    Swal.fire('Error', 'Error al cargar las entradas', 'error');
                });
        }
        ...
        function mostrarEntradas() {
            ...
            entradasData.forEach(entrada => {
                ...
                row.innerHTML = `
                    <td>#${entrada.id}</td>
                    <td>${fecha}</td>
                    <td><i class="fas fa-box"></i> ${entrada.total_productos.toLocaleString('en-US') || 0}</td>
                    <td><i class="fas fa-cubes"></i> ${parseFloat(entrada.total_cantidad || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td>RD$ ${parseFloat(entrada.total_costo || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                    <td>${estadoBadge}</td>
                    <td>
                        <div class="actions">
                            <a href="inventario-entrada-detalle.php?id=${entrada.id}" class="btn btn-info" title="Ver detalles">
                                <i class="fas fa-eye"></i> Ver
                            </a>
                            ${entrada.estado === 'activo' ? `
                                <button class="btn btn-danger" onclick="cancelarEntrada(${entrada.id})" title="Cancelar entrada">
        ...
        function actualizarEstadisticas() {
            ...
            entradasData.forEach(entrada => {
                if(entrada.estado === 'activo') {
                    totalEntradas++;
                    totalProductos += parseInt(entrada.total_productos || 0);
                    totalValor += parseFloat(entrada.total_costo || 0);
                    
                    const fechaEntrada = new Date(entrada.fecha);
                    if(fechaEntrada.getMonth() === mesActual && fechaEntrada.getFullYear() === añoActual) {
                        totalMes++;
                    }
                }
            });
            
            document.getElementById('stat-total').textContent = totalEntradas.toLocaleString('en-US');
        ...
        function cancelarEntrada(id) {
            Swal.fire({
                ...
                html: `
                    <p>Esta acción revertirá el inventario agregado.</p>
