...
$sql_base = "SELECT
            ...
            ON c.id = ci.id_cliente
            INNER JOIN empleados AS e
            ON e.id = ci.id_empleado
            WHERE 1=1";
...
if (!empty($filtro_buscador)) {
    $sql_base .= " AND (ci.no LIKE ? OR CONCAT(c.nombre, ' ', c.apellido) LIKE ? OR CONCAT(e.nombre, ' ', e.apellido) LIKE ?)";
...
$sql_count = "SELECT COUNT(*) as total FROM ($sql_base GROUP BY ci.no) AS subquery";
...
$sql = "$sql_base GROUP BY ci.no ORDER BY ci.fecha DESC LIMIT ?, ?";
...
<body>
    ...
    <?php 
        if (isset($_GET['error']) && $_GET['error'] == 'missing_no') {
            echo "<script>
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'No se encontró la cotizacion seleccionada.',
                    confirmButtonText: 'Aceptar'
                }).then(() => {
                    window.location.href = 'cotizacion-registro.php';
                });
            </script>";
        }
    ?>
                    ...
                    <div class="card">
                        ...
                        <div class="search-bar">
                            <div class="search-input">
                                ...
                                <input type="text" id="buscador" name="buscador" value="<?php echo $filtro_buscador; ?>" placeholder="Buscar cotización por número, cliente o vendedor">
                            </div>
                            <button class="btn btn-primary" type="submit">
                                <i class="fas fa-search"></i>
                                Buscar
                            </button>
                            <button class="btn btn-secondary" type="reset" onclick="window.location.href='cotizacion-registro.php'">
                            ...
                            <?php
                                if ($results->num_rows > 0) {
                                    while ($row = $results->fetch_assoc()) {
                                        // FORMATO DE MONEDA
                                        $totalc = number_format($row['total'], 2, '.', ',');
                                        ...
                                        echo "
                                            <tr>
                                                ...
                                                <td>RD$ {$totalc}</td>
                                                <td><span class='status status-{$estadoClass}'>{$row['estado']}</span></td>
                                                <td><button class='btn btn-secondary' onclick=\"window.location.href='cotizacion-detalle.php?no={$row['no']}'\">Ver Detalles</button></td>
                                            </tr>
                                        ";
                                    }
                                } else {
                    ...
                    <div class="mobile-cards-grid">
                        ...
                        if ($results1->num_rows > 0) {
                            while ($row1 = $results1->fetch_assoc()) {
                                // FORMATO DE MONEDA
                                $totalc1 = number_format($row1['total'], 2, '.', ',');

                                // Determinar la clase CSS del estado
                                $estadoClass1 = "";
                                if ($row1['estado'] == "vendida") {
                                    $estadoClass1 = "paid";
                                } elseif ($row1['estado'] == "pendiente") {
                                    $estadoClass1 = "pending";
                                } elseif ($row1['estado'] == "cancelada") {
                                    $estadoClass1 = "cancel";
                                }
                        ?>
                                <!-- Factura individual -->
                                <div class="invoice-card">
                                    <div class="invoice-card-header">
                                        ...
                                        <span class="status status-<?php echo $estadoClass1; ?>"><?php echo $row1['estado']; ?></span>
                                    </div>
                                    <div class="invoice-card-body">
                                        <div class="invoice-detail">
                                            <span class="detail-label">Cliente</span>
                                            <span class="detail-value"><?php echo $row1['nombreCliente']; ?></span>
                                        </div>
                                        <div class="invoice-detail">
                                            <span class="detail-label">Vendedor</span>
                                            <span class="detail-value"><?php echo $row1['nombreEmpleado']; ?></span>
                                        </div>
                                        <div class="invoice-detail">
                                        ...
                                        <div class="invoice-detail">
                                            ...
                                            <span class="detail-value">RD$ <?php echo $totalc1; ?></span>
                                        </div>
                                        <div class="">
                                            <button class='btn btn-secondary' onclick="window.location.href='cotizacion-detalle.php?no=<?php echo $row1['no']; ?>'">Ver Detalles</button>
                                        </div>
                                    </div>
                                </div>
                        <?php
